...
class AnnouncementViewSetTestCase(BaseAPITestCase):
    ...
    def test_authorized_create(self):
        ...
        self.assertEquals(response.json()["fromGroup"]["id"], 11)
        self.assertEquals(response.json()["message"], message)
        self.assertEquals(len(response.json()["groups"]), 1)
        self.assertEquals(response.json()["groups"][0]["id"], 2)
        self.assertEquals(len(response.json()["events"]), 1)
        self.assertEquals(response.json()["events"][0]["id"], 1)
    ...
    def test_authorized_list_own(self):
        ...
        self.assertEquals(response.json()["results"][0]["id"], 6)
